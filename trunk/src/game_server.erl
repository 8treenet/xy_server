...
%% @doc @todo Add description to logic_server.
...
handle_cast({player_send, Pid, Bin}, State) ->
 	gen_server:cast({gateway_server, State#state.gateway_node}, {player_send,Pid,Bin}),
    {noreply, State};
handle_cast({register_gateway, Node}, State) ->
 	NewState = #state{gateway_node = Node},
    {noreply, NewState};
...
send(PID, Bin)->
	gen_server:cast(?MODULE, {player_send, PID, Bin}).